    ...
    <div className="space-y-5 pb-8">
      ...
      {totalDebts.length > 0 && (
        <div className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
          ...
          <ul className="space-y-3">
            {totalDebts.map((debt) => (
              <PersonDebtItem key={debt.personId} debt={debt} />
            ))}
          </ul>
        </div>
      )}
...
function PersonDebtItem({ debt }: { debt: PersonDebt }) {
  // Inicializamos todas las casillas marcadas por defecto
  const [selectedMonths, setSelectedMonths] = useState<Set<string>>(() => {
    return new Set(debt.months.map(m => m.monthKey));
  });
  ...
  const isAllSelected = selectedMonths.size === debt.months.length;
  
  let payingNowTotal = 0;
  let pendingTotal = 0;
  ...
  debt.months.forEach(m => {
    if (selectedMonths.has(m.monthKey)) {
      payingNowTotal += m.amount;
      selectedLabels.push(formatMonthLabel(m.monthKey));
    } else {
      pendingTotal += m.amount;
      pendingLabels.push(formatMonthLabel(m.monthKey));
    }
  });
  ...
  const handleWhatsApp = async () => {
    let text = "";
    if (isAllSelected) {
      text = `Hola ${debt.name} 👋, te escribo para recordarte que tienes un saldo pendiente de *${formatMoney(debt.totalDebt)}* de las vaquitas mensuales. 🐮\n\nCorresponde a los meses de: ${selectedLabels.join(", ")}.`;
    } else {
      text = `Hola ${debt.name} 👋, del saldo acumulado que tienes de ${formatMoney(debt.totalDebt)}, acordamos que abonarás ahora *${formatMoney(payingNowTotal)}* (correspondiente a: ${selectedLabels.join(", ")}). 🐮\n\nQuedará un saldo pendiente de *${formatMoney(pendingTotal)}* para los meses de: ${pendingLabels.join(", ")}.`;
    }
    await shareWhatsAppText(text);
  };

  return (
    <li className="flex flex-col gap-3 rounded-xl bg-white p-4 shadow-sm md:flex-row md:items-start md:justify-between">
      <div className="flex-1">
        <p className="font-semibold text-slate-900">{debt.name}</p>
        <p className="text-sm text-slate-600 mb-3">
          Deuda Histórica: <strong className="text-amber-700">{formatMoney(debt.totalDebt)}</strong>
        </p>
        
        <div className="space-y-2 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Desglose de meses</p>
          {debt.months.map(m => (
            <label key={m.monthKey} className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={selectedMonths.has(m.monthKey)}
                onChange={() => toggleMonth(m.monthKey)}
                className="h-4 w-4 rounded border-slate-300 text-amber-600 focus:ring-amber-500"
              />
              <span className="text-sm text-slate-700 capitalize flex-1">{formatMonthLabel(m.monthKey)}</span>
              <span className="text-sm font-medium text-slate-900">{formatMoney(m.amount)}</span>
            </label>
          ))}
        </div>
        ...
        {!isAllSelected && (
          <div className="flex gap-4 text-sm border-t border-slate-100 pt-3">
            <div>
              <span className="block text-xs text-slate-500">Paga ahora</span>
              <strong className="text-emerald-600">{formatMoney(payingNowTotal)}</strong>
            </div>
            <div>
              <span className="block text-xs text-slate-500">Queda pendiente</span>
              <strong className="text-amber-700">{formatMoney(pendingTotal)}</strong>
            </div>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="mt-2 flex shrink-0 items-center justify-center gap-2 rounded-lg bg-[#25D366] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#20bd5a] disabled:opacity-50 md:mt-0"
      >
